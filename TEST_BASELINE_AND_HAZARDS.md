...
| `tests/test_webgpu_ssa_backend.py::test_ast_generated_float32_program_emits_wgsl_compute` | **1 failed** | Deprecated `compile_ast_aot` returns an empty captured numerical program, so WGSL correctly reports `compute module has no named output`. Verified 2026-08-20 in a clean worktree at `ab9a078`; direct repository-SSA WebGPU emission and the browser benchmark paths are unaffected. |
