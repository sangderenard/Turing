...
def main(argv=None, *, recording=None, live_step=None,
    ...
    plate = None
    cold_tip = None
    if hasattr(rec, "machine_graph"):
        engine_toy = HERE.parents[1] / "engine_toy"
        if str(engine_toy) not in sys.path:
            sys.path.insert(0, str(engine_toy))
        from vehicle_mesh import build_drivetrain_solid_parts
        tip_name = f"node_{rec.cold_tip_identity.replace('.', '_')}"
        part = next(p for p in build_drivetrain_solid_parts(rec.machine_graph)
                    if p.name == tip_name)
        cold_tip = Mesh(part.vertices, part.normals, 2)
    else:
    ...
    while True:
        ...
        if draw_atmosphere:
            rain.update(frame, dt_wall * args.speed if playing else 0.0,
                        col_w, col_h, col_d)
        ...
        if plate is not None:
            opaque.append((plate, m_plate))
        if cold_tip is not None:
            opaque.append((cold_tip, ident))
        for mesh, m in opaque:
        ...
        rain_mesh = rain.build_mesh(view_dir) if draw_atmosphere else None
        ...
        if draw_atmosphere:
            ...
            renderer.draw_mesh(
                stacks[axis].vao, stacks[axis].n,
                gl(proj @ view @ m_column), gl(view @ m_column),
