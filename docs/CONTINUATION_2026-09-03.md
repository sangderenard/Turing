...
Branch `codex/recursive-reduction-bridge` (the turing/ nested repo; the outer C:\dev\Powershell repo is on `nogodsnomasters`). Everything below is committed by the commit that
