"""Turn the full-domain sine demo into a width x digits x backend sweep."""
from pathlib import Path

PATH = Path(__file__).resolve().parents[2] / "tools/demo_full_domain_sine.py"
...
def main() -> None:
    ...
    if "def build_source(" in text:
        raise SystemExit("already a sweep; refusing to re-apply")
    # Keep everything up to the coefficient helpers, then rewrite the body.
    cut = text.index("sin_coefficients = core_terms(")
    head = text[:cut]
    head = head.replace(
