"""Compile a full-domain sine: wide reduction, proven core, quadrant blend.

The shape is the classic one and the widths are the point. The quadrant
index is an integer and stays a double -- exactly representable, and
computed from the collapsed argument. The SUBTRACTION of that many
quarter-turns is where the digits cancel, so it runs in limbs against a
tau derived to the same width. The core then sees an argument inside the
octant it was proven on, and both cores are evaluated and blended by
masks rather than branched on, so every lane emits straight-line code.
"""
import sys
