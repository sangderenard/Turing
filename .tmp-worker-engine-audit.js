...
function requestParametricVehiclePipelines(body,reason){
  const gpu=vehicleGpu;if(!gpu?.residentGraph||!gpu.engineProfileVariants?.has(Number(body.engineProfileSelector||0)))return false;
  body.gpuKernelVariant="engine-profile-static";body.gpuStateDirty=true;
  postMessage({type:"vehicle-gpu-recovered",reason:`precompiled static profile switch · ${reason}`});return true;
}
function residentVehicleControlValues(body){
  ...
  return new Float32Array([throttle,Number(body.appliedSteering??body.controls?.steering??0),
    ...
    body.bodyShell==="bare-frame"||(body.config.mechanical_graph?.edges||[]).filter(edge=>edge.identity.startsWith(
      body.bodyShell==="five-turret-carrier"?"armor.mount.":"body_shell.mount."))
...
function residentVehicleStep(body,dt){
  ...
  try{const encoder=gpu.device.createCommandEncoder(),run=(pipeline,bindGroup,dispatch)=>{const pass=encoder.beginComputePass();
    ...
    const engineVariant=gpu.engineProfileVariants?.get(Number(body.engineProfileSelector||0)),integrationPipelines=
      engineVariant?.pipelines||gpu.integrationPipelines,integrationBindGroups=engineVariant?
      gpu.engineProfileBindGroups.get(Number(body.engineProfileSelector||0)):gpu.integrationBindGroups,
      integrationStages=engineVariant?.stages||gpu.integrationStages;
    integrationStages.forEach((stage,index)=>run(integrationPipelines[index],integrationBindGroups[index],
...
function ensureVehicleEnergy(body){
  if(body.energy)return body.energy;const fuel=body.config.fuel_system||{},electrical=body.config.electrical||{},seed=body.energySeed||{},
    ...
    ammoMassKg=body.bodyShell==="five-turret-carrier"?ammoCount*Number(ammunition.round_mass_kg||0):0;
...
function updateVehicleOutriggers(body,dt){
  const assembly=(body.bodyAssemblies||[]).find(item=>item.identity===body.bodyShell),spec=assembly?.outriggers,
    energy=ensureVehicleEnergy(body);if(!spec){energy.outriggerExtension=0;energy.outriggerAnchors={};energy.outriggerHydraulicPowerW=0;return;}
  const runtime=body.outriggers||={commanded:false,extension:0,anchors:{},previousFeet:{}},rate=Math.max(.01,Number(spec.extension_rate_m_s||.34)),
    maximum=Math.max(.1,Number(spec.maximum_extension_m||1.1)),target=runtime.commanded?maximum:0,step=rate*dt;
  runtime.previousFeet||={};runtime.anchors||={};
  runtime.extension+=Math.sign(target-runtime.extension)*Math.min(Math.abs(target-runtime.extension),step);
  const ch=body.config.chassis,halfLength=Number(ch.half_length),halfWidth=Number(ch.half_width),names=spec.feet||[],
    corrections=[],rotations=[];
  for(const name of names){const front=name.startsWith("front")?1:-1,left=name.endsWith("left")?-1:1,
    ...
    const desiredMount=anchor.position.map((value,index)=>value-anchor.direction[index]*length),error=desiredMount.map(
      (value,index)=>value-mount[index]);corrections.push(error);rotations.push([
        mountOffset[1]*error[2]-mountOffset[2]*error[1],mountOffset[2]*error[0]-mountOffset[0]*error[2],
        mountOffset[0]*error[1]-mountOffset[1]*error[0]]);body.contactRuntimePartId=anchor.runtimePartId||body.contactRuntimePartId;
  }
  if(corrections.length){const correction=[0,1,2].map(axis=>corrections.reduce((sum,item)=>sum+item[axis],0)/corrections.length),
      gain=Math.min(1,dt*18);for(let axis=0;axis<3;axis++){body.position[axis]+=correction[axis]*gain;
        body.velocity[axis]*=Math.max(0,1-dt*(8+corrections.length*2));}
    const angular=[0,1,2].map(axis=>rotations.reduce((sum,item)=>sum+item[axis],0)/rotations.length),angularGain=Math.min(.18,dt*2.8);
    body.roll+=angular[0]*angularGain;body.yaw+=angular[1]*angularGain;body.pitch+=angular[2]*angularGain;
    body.rollVelocity*=Math.max(0,1-dt*12);body.yawVelocity*=Math.max(0,1-dt*12);body.pitchVelocity*=Math.max(0,1-dt*12);
  ...
  energy.outriggerExtension=runtime.extension;energy.outriggerAnchors=Object.fromEntries(Object.entries(runtime.anchors).map(
    ...
    Number(spec.hydraulic_power_w||820):0;
...
function prepareVehicleEnergy(body,dt){
  ...
  if(body.bodyShell==="five-turret-carrier"&&state.computerOnline)state.electricalLoadW+=Number((body.bodyAssemblies||[]).find(
...
function applyVehicleWheelPart(body,part){
  ...
  const assemblyInertia=(mass,r,wd,rr)=>{const localDiametral=mass*(3*r*r+wd*wd)/12,localPolar=.5*mass*r*r,
      wb=Number(wheels.wheelbase_half_length),track=Number(wheels.track_half_width),
      y=-Number(body.config.chassis.clearance)-Number(body.config.suspension.rest_length)+r;
    return[4*(localDiametral+mass*(y*y+track*track)),4*(localDiametral+mass*(wb*wb+track*track)),
      4*(localPolar+mass*(wb*wb+y*y))];},referenceMass=reference.wheelMassKg+reference.tireMassKg,currentMass=wheelMassKg+tireMassKg,
...
function updateVehicleDamage(body,dt){
  ...
  const impactLoad=Number(c.mass||0)*Math.max(0,Number(body.accelerationMagnitude||0)-18),edges=c.mechanical_graph?.edges||[],
    ...
    intactShellMounts=Math.max(1,edges.filter(edge=>(edge.identity.startsWith("body_shell.mount.")||
      body.bodyShell==="five-turret-carrier"&&(edge.identity.startsWith("armor.mount.")||edge.identity.startsWith("turret.mount.")))&&
  ...
  for(const edge of edges){const state=damage.members[edge.identity];if(!state||state.failed)continue;
    const corner=names.find(name=>edge.identity.includes(name)),cornerIndex=corner?names.indexOf(corner):-1,
      shellMount=edge.identity.startsWith("body_shell.mount.")||body.bodyShell==="five-turret-carrier"&&(
...
function radialTireContact(body,worldHub,forwardAxis,rightAxis,steer,camber,caster,radius,width,travel,reach,angular,dt){
  const cs=Math.cos(steer),ss=Math.sin(steer),cc=Math.cos(caster),sc=Math.sin(caster),baseDown=rotateBodyVector(body,[0,-1,0]),
    casterForward=forwardAxis.map((value,index)=>value*cc-baseDown[index]*sc),
    casterDown=baseDown.map((value,index)=>value*cc+forwardAxis[index]*sc),rollingAxis=[
      casterForward[0]*cs+rightAxis[0]*ss,casterForward[1]*cs+rightAxis[1]*ss,casterForward[2]*cs+rightAxis[2]*ss],
    ...
    down=casterDown.map((value,index)=>value*cm-rawAxle[index]*sm),
  ...
  for(const angle of radialAngles){const ca=Math.cos(angle),sa=Math.sin(angle),radial=[
    ...
    for(const lateralFraction of lateralFractions){const index=probeIndex++,probe=[
      ...
      spatialHit=terrainSegmentCrossing(worldHub,probe,body,reach)||solidSegmentCrossing(worldHub,probe,body),
      temporalHit=spatialHit?null:(terrainSegmentCrossing(probe,nextProbe,body,reach)||solidSegmentCrossing(probe,nextProbe,body)),
...
function wheelContactRecords(body,dt){
  ...
  return names.map((name,i)=>{const o=offsets[i],localHub=graphPositions?.get(`suspension.${name}.hub`)||
    ...
    radialContact=radialTireContact(body,worldHub,forwardAxis,rightAxis,steer,camber,caster,
...
function vehicleExteriorContactSamples(body){
  ...
  if(body.bodyShell!=="bare-frame"){const chassis=body.config.chassis,halfLength=Number(chassis.half_length),
    ...
    if(body.bodyShell==="five-turret-carrier"){for(const x of [-.91,0,.91])for(const z of [-1.04,1.04])
...
function updateVehicleChassisLeveling(body,dt){
  ...
  for(const edge of body.config.mechanical_graph?.edges||[]){if(!edge.linear_actuator)continue;
    ...
    body.linkLengthModifiers[edge.identity]=edgeLimit*normalized*(isTieRod ? .36 : .74);}
...
onmessage=async event=>{const m=event.data||{};
  if(m.type==="init"){const made=await WebAssembly.instantiate(m.wasm,{});instance=made.instance;abi={...m.abi,entrypoint:m.entrypoint};parameters=m.parameters||{};
    ...
    initializeVehicleGpu(m.vehicleWebgpu).then(gpu=>{vehicleGpu=gpu;
      ...
      for(const body of bodies.values())if(body.kind==="vehicle"){body.gpuKernelVariant="engine-profile-static";
    ...
    bodies.set(m.body.identity,body);if(body.kind==="vehicle"){body.engineProfileSelector=Number(body.engineProfileSelector||0);
      body.gpuKernelVariant=vehicleGpu?.engineProfileVariants?"engine-profile-static":"parametric-pending";
    ...
    b.bodyShell=String(m.shellIdentity||"bare-frame");const energy=ensureVehicleEnergy(b),assembly=m.assembly||
      ...
      capacity=Math.max(0,Math.min(Number(ammunition.capacity_count||0),Math.floor(Number(ammunition.capacity_mass_kg||0)/
        Math.max(1e-9,Number(ammunition.round_mass_kg||1))),Math.floor(Number(ammunition.capacity_volume_m3||0)/
        Math.max(1e-9,Number(ammunition.round_volume_m3||1))))),
      count=b.bodyShell==="five-turret-carrier"?Math.max(0,Math.min(capacity,Number(m.ammoCount||0))):0;
