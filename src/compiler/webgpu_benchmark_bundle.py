...
const feedBuffers=feeds.slice(0,kernel.arity).map(v=>buffer(v,GPUBufferUsage.STORAGE));const output=device.createBuffer({size:count*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC});let scalarBuffer=null;const entries=[...feedBuffers.map((b,i)=>({binding:i,resource:{buffer:b}})),{binding:kernel.arity,resource:{buffer:output}}];if(kernel.kind==='gemm'){scalarBuffer=buffer(new Float32Array([1,0]),GPUBufferUsage.UNIFORM);entries.push({binding:3,resource:{buffer:scalarBuffer}});}const bind=device.createBindGroup({layout:pipeline.getBindGroupLayout(0),entries});
