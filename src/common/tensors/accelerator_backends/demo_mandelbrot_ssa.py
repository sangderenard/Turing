"""Copied Mandelbrot compiler demo stopped at Turing IR and validated as SSA.
...
def animate_glsl(
    *,
    ...
    try:
        ...
        print(
            "observe : canonical NumPy values; no GLSL realization/context",
            flush=True,
        )
        print(
        ...
        # Discovery needs one lexical execution of the planned outer loop,
        # not eager numerical construction of every frame in a runtime batch.
        # The control IR owns repetition and the resident workgroup extent;
        # replaying multiple frames here only multiplies host capture work.
        compile_stop = 1
        phase_started = time.perf_counter()
        deployment.capture_fused_programs(
            batch_feeds(0, compile_stop),
            precompile_only=True,
        )
        capture_precompile_seconds = time.perf_counter() - phase_started
        print(
            "prepare : "
            f"structural {structural_compile_seconds:.3f}s | "
            f"capture/precompile {capture_precompile_seconds:.3f}s | "
            "GLSL realization disabled",
            flush=True,
        )
        if program_table:
        ...
        if precompile is None or control is None:
            raise RuntimeError(
                "the copied demo reached no Turing numerical/control "
                "precompile boundary"
            )
        region_programs = {}
        for region_key, captured in (
            deployment.captured_region_programs.items()
        ):
            region_index = (
                region_key[-2]
                if isinstance(region_key, tuple)
                and len(region_key) >= 2
                else region_key
            )
            if isinstance(region_index, bool) or not isinstance(
                region_index, int
            ):
                raise RuntimeError(
                    f"invalid precompile region key {region_key!r}"
                )
        ...
        report_lines = [
            "Mandelbrot copied-demo Turing precompile -> SSA",
            f"ProcessGraph nodes: {sum(shell.source_node_count for shell in planned_shells)}",
            f"scheduled nodes: {sum(shell.primitive_count for shell in planned_shells)}",
            f"deployment regions: {sum(shell.dispatch_count for shell in planned_shells)}",
            f"function shells: {len(planned_shells)}",
            f"precompile steps: {len(precompile.program.steps)}",
            (
                "precompile format: valid"
                if lowering.validation.valid_precompile
                else "precompile format: invalid"
            ),
            (
                "SSA compatibility: complete"
                if lowering.validation.ssa_compatible
                else "SSA compatibility: shortfalls"
            ),
            *(
                (
                    "precompile format issues:",
                    *(
                        f"- {issue.format()}"
                        for issue in lowering.validation.format_issues
                    ),
                )
                if lowering.validation.format_issues
                else ()
            ),
            ...
            *(
                ...
                for cycle in lowering.cycles
            ),
        ]
        if not lowering.cycles:
            report_lines.append("- none")
        report = "\n".join(report_lines) + "\n"
        report_path = destination.with_suffix(".ssa-report.txt")
        dictionary_path = destination.with_suffix(".ssa.json")
        report_path.write_text(report, encoding="utf-8")
        dictionary_path.write_text(
            json.dumps(
                ssa_module_dictionary(lowering.module),
                indent=2,
                sort_keys=True,
                default=str,
            )
        ...
        print(
            f"SSA report: {report_path}\n"
            f"SSA dictionary: {dictionary_path}",
            flush=True,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.animate:
        ap.error(
            "the SSA-validation copy supports the compiler demo's "
            "--animate path only"
        )
    if args.only_glsl:
        args.shell = "glsl"
