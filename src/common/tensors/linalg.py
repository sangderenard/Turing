...
def _forward_substitute(LU: AbstractTensor, b: AbstractTensor) -> AbstractTensor:
    ...
    y = b.clone()
    for i in range(n):
        # i == 0 needs no special case: its prefix mask is empty, so the
        # product is zero and the row passes through unchanged.
...
def solve(A: AbstractTensor, b: AbstractTensor) -> AbstractTensor:
    ...
    if b.dim() == len(shpA) - 1:  # (..., n)
        # A trailing axis of one, which is what the reshape spelled by
        # rebuilding the shape tuple in Python always meant.
        b = b.unsqueeze(-1)
        squeeze_vec = True
    ...
    return x.squeeze(-1) if squeeze_vec else x
